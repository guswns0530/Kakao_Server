...
select
    ...
    nvl(A.NAME, LISTAGG(nvl(DECODE(C.CUTOFF_RS, 1, C.nickname, null), D.NAME), ', ') within group ( order by C.nickname, D.NAME)) as ROOM_NAME,
    nvl(A.NAME, LISTAGG(D.user_id, ', ') within group ( order by C.nickname, D.NAME)) as USER_IDS,
